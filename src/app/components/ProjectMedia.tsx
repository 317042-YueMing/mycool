  ...
  return (
    <div className="project-media">
      ...
      <ImagePreview capture={capture} title={`Mycool · ${capture.label}截图`} description="V2 页面记录 · Refero 重构前" className={`mycool-media media-surface view-${view}`} label={`放大查看 Mycool ${capture.label}截图`}>
        {view === "desktop" ? (
          <>
            <span className="desktop-capture"><Image src={captures.desktop.src} width={captures.desktop.width} height={captures.desktop.height} alt={captures.desktop.alt} sizes="(max-width: 700px) 100vw, 65vw" /></span>
            <span className="mobile-capture"><Image src={captures.mobile.src} width={captures.mobile.width} height={captures.mobile.height} alt={captures.mobile.alt} sizes="(max-width: 700px) 35vw, 16vw" /></span>
          </>
